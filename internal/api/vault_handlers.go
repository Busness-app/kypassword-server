...
func (s *Server) handleVaultDownload(w http.ResponseWriter, r *http.Request, u users.User) {
	...
	_, _ = io.Copy(w, rc)
	_, _ = s.audit.Log(r.Context(), "vault.download", u.ID, "", clientIP(r), fmt.Sprintf("downloaded vault v%d", meta.Version))
}
...
func (s *Server) handleVaultUpload(w http.ResponseWriter, r *http.Request, u users.User) {
	...
	if err != nil {
		...
		if errors.As(err, &confErr) {
			_, _ = s.audit.Log(r.Context(), "vault.conflict_rejected", u.ID, devID, clientIP(r), confErr.Error())
	...
	_, _ = s.audit.Log(r.Context(), "vault.saved", u.ID, devID, clientIP(r), fmt.Sprintf("saved vault v%d", meta.Version))
...
func (s *Server) handleVaultEnvelopes(w http.ResponseWriter, r *http.Request, u users.User) {
	...
	_, _ = s.audit.Log(r.Context(), "vault.envelopes_updated", u.ID, "", clientIP(r), "updated key envelopes")
...
func (s *Server) handleVaultHistoryRestore(w http.ResponseWriter, r *http.Request, u users.User) {
	...
	_, _ = s.audit.Log(r.Context(), "vault.restored_snapshot", u.ID, "", clientIP(r), fmt.Sprintf("restored snapshot %s to v%d", id, meta.Version))
	writeJSON(w, http.StatusOK, map[string]any{
		"ok":       true,
		"metadata": meta,
	})
...
func (s *Server) handleVaultConflictDiscard(w http.ResponseWriter, r *http.Request, u users.User) {
	...
	_, _ = s.audit.Log(r.Context(), "vault.conflict_discarded", u.ID, "", clientIP(r), "discarded conflict "+id)
	writeJSON(w, http.StatusOK, map[string]any{"ok": true})
}
